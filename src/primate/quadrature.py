...
def lanczos_quadrature(
	...
	**kwargs: dict,
	...
	where $d \mu(x) = w(x) dx$ denotes the appropriate measure and $f(x)$ represents the function being approximated.
	The limits $a,b$ and weight function $w(x)$ depend on how `J` was constructed. When $J$ arises from the Lanczos
	...
	In this sense, the nodes $\lambda_i$ and weights $\w_i$ constructed by this function can approximate
	any spectral sum by post-processing with some $f$.
