...
Of course, the aforementioned baseline is not useful, as an explicit expression of the matrix polynomial is already available ($A + \epsilon I$). However, there many situations where the matrix polynomial corresponding to $f$ is analytically intractable, expensive to obtain, or simply unknown. For example, consider the map: 
...
There is a larger degree of error compared to the base as evidenced by the $\lVert \cdot \rVert_\infty$-normed difference between `v_truth` and `v_test`. This is to be expected, as in general approximating the action $v \mapsto A^{-1} v$ will always be more difficult that $v \mapsto A v$, even if $A$ is well-conditioned.
