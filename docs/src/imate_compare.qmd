...
[^3]: In theory, there is nothing stopping on from using e.g. GPU-based tensor libraries to accelerate matrix-vector products. Indeed, `primate` was designed to work with essentially any operator matching the interface, see the [C++ integration guide](advanced/cpp_integration.qmd). However, nativ GPU support in the form of CUDA- or ROCm-implemented Linear operators is not provided.
