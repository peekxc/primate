...
`primate`'s namesake (and some of the original code[^1]) was inspired from the (excellent) [`imate` package](https://github.com/ameli/imate), prompting questions about their differences. In general, `primate` was developed with slightly different goals in mind than `imate`, most of which have to do with things like integrability, genericity, and choice of FFI / build system.
...
These modifications not only simplify access to the SLQ method from *user* (i.e. dependent) packages, but they enable native support for arbitrary types adhering to SciPys [`LinearOperator`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.linalg.LinearOperator.html) abstract interface. For more details on this, see the integration guide. 

[^1]: Before v0.2, much of `primate`'s code was essentially ported and refactored from `imate`. The code for v0.2+ has been re-written using [Eigen](https://eigen.tuxfamily.org/index.php?title=Main_Page) the library.
