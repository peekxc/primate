...
`primate` relies on the use of C++20's [concepts](https://en.cppreference.com/w/cpp/language/constraints) to support calling the SLQ trace estimator using _any_ matrix function and _any_ linear operator having the preqrequisite member functions. To understand this more in detail, examples are given below. 
...
|---------------------------|-------------------------|------------------------------|------------------------|
| `LinearOperator`          | $o = o + A v$           | `A.matvec(v, o)`            | NA                     |
| `AdjointOperator`         | $o = o + A^T v$         | `A.rmatvec(v, o)`           | `LinearOperator` |
| `LinearAdditiveOperator`  | $o \gets o + \alpha Av$ | `A.matvec_add(v, alpha, o)` | `LinearOperator` |
