...
#| echo: true
#| output: true
p = figure(width=500, height=200, title="Spectrum of A")
...
If built-in, the basic way to approximate the trace of a matrix function is to simply supply the name of the spectral function to be applied. For example, to approximate the log-determinant:

```{python}
from primate.trace import hutch

## Random positive definite matrix
A = symmetric(n=300, pd=True)
...
Even using $\frac{n}{12}$ matvecs, we get a decent approximation. But how good is this estimate? 

To get a slightly better idea, you can set `verbose=True`:

```{python}
hutch(A, fun="log", maxiter=25, verbose=True)
```
